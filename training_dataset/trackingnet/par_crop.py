...
dataset_path = '/media/soonki/backup/trackingnet'
# dataset_path = './data'
sub_sets = ['TRAIN_0', 'TRAIN_1', 'TRAIN_2', 'TRAIN_3', 'TRAIN_4', 'TRAIN_5', 'TRAIN_6', 'TRAIN_7', 'TRAIN_8', 'TRAIN_9', 'TRAIN_10', 'TRAIN_11']
#sub_sets = ['TRAIN_11']
#sub_sets = ['TRAIN_0', 'TRAIN_1', 'TRAIN_2', 'TRAIN_3', 'TRAIN_4', 'TRAIN_5', 'TRAIN_6', 'TRAIN_7', 'TRAIN_8', 'TRAIN_9', 'TRAIN_10']
#sub_sets = ['TEST']
...
def crop_video(video, d_set, crop_path, instanc_size):
    if video != 'list.txt':
        ...
        gt_path = join(dataset_path, d_set, 'anno', video + '.txt')
        images_path = join(dataset_path, d_set, 'zips', video)
        f = open(gt_path, 'r')
        ...
        for idx, gt_line in enumerate(groundtruth):
            ...
            im = cv2.imread(join(images_path, str(idx)+'.jpg'))
...
def main(instanc_size=511, num_threads=14):
    crop_path = '/media/soonki/backup/trackingnet_crop/crop{:d}'.format(instanc_size)

    if not isdir(crop_path): mkdir(crop_path)
    for d_set in sub_sets:
        save_path = join(crop_path, d_set)
        videos = listdir(join(dataset_path, d_set, 'zips'))
        ...
        with futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            ...
            for i, f in enumerate(futures.as_completed(fs)):
                ...
                printProgress(i, n_videos, prefix='train:'+d_set, suffix='Done ', barLength=40)
