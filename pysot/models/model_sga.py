...
class Graph_Attention_Union(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        # aggregated feature
        self.fi_1 = nn.Sequential(
            nn.Conv2d(in_channel * 2, out_channel, 1, 1),
            nn.BatchNorm2d(out_channel),
            nn.ReLU(inplace=True),
        )
        self.fi_2 = nn.Sequential(
            nn.Conv2d(in_channel * 2, out_channel, 1, 1),
            nn.BatchNorm2d(out_channel),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, zf, xf):
        ...
        # [b, 256, 169] x [b, 169, 256] = [b, 256, 256]
        zf_channel_weight = torch.matmul(zf_trans_plain, zf_g_plain)
        # [b, 625, 256] x [b, 256, 169] = [b, 625, 169]
        pixel_weight = torch.matmul(xf_trans_plain, zf_trans_plain)
        # [b, 625, 169]对169上进行softmax
        pixel_weight = F.softmax(pixel_weight, dim=2)
        # [b, 256, 256]对第二个256维度进行softmax
        channel_weight = F.softmax(zf_channel_weight, dim=2)
        # [b, 625, 169] x [b, 169, 256] = [b, 625, 169] ==> [b, 256, 625]
        embedding_pixel_weight = torch.matmul(pixel_weight, zf_g_plain).permute(0, 2, 1)
        # [b, 256, 25, 25]
        embedding_pixel_weight = embedding_pixel_weight.view(-1, shape_x[1], shape_x[2], shape_x[3])

        # [b, 256, 256] x [b, 256, 25, 25].view(b, 256, 625) = [b, 256, 625]
        xf_trans_view = xf_trans.view(-1, shape_x[1], shape_x[2] * shape_x[3])
        embedding_channel_weight = torch.matmul(channel_weight, xf_trans_view)
        # [b, 256, 25, 25]
        embedding_channel_weight = embedding_channel_weight.view(-1, shape_x[1], shape_x[2], shape_x[3])

        # aggregated feature
        E_channel = self.fi_1(torch.cat([embedding_channel_weight, xf_g], dim=1))
        E_pixel = self.fi_2(torch.cat([embedding_pixel_weight, xf_g], dim=1))
        output = self.fi(torch.cat([E_channel, E_pixel], dim=1))
...
class ModelBuilder(nn.Module):
    ...
    def forward(self, data):
        ...
        return outputs, cls_loss, loc_loss, cen_loss
