...
class Inception3_ou(nn.Module):
    ...
    def __init__(self):
        super(Inception3_ou, self).__init__()
    ...
    def forward(self, x, bbox=torch.tensor([])):
        ...
        # [0~255] ==> [-128~+127] ==> [-1~+1]
        # 类似BatchNorm
        # TODO:是否真的需要对图片进行对称区域的正则？
        x_ch0 = (torch.unsqueeze(x[:, 2], 1) - bias) / bias
        x_ch1 = (torch.unsqueeze(x[:, 1], 1) - bias) / bias
        x_ch2 = (torch.unsqueeze(x[:, 0], 1) - bias) / bias
        # [batch, 3, 127, 127] 或者 [batch, 3, 287, 287]
        x = torch.cat((x_ch0, x_ch1, x_ch2), 1)

        # [batch, 32, 63, 63] 或者 [batch, 32, 143, 143]
        x = self.Conv2d_1a_3x3(x)

        # [batch, 32, 61, 61] 或者 [batch, 32, 141, 141]
        x = self.Conv2d_2a_3x3(x)

        # [batch, 64, 61, 61] 或者 [batch, 64, 141, 141]
        x = self.Conv2d_2b_3x3(x)

        # [batch, 64, 30, 30] 或者 [batch, 64, 70, 70]
        x = F.max_pool2d(x, kernel_size=3, stride=2)

        # [batch, 80, 30, 30] 或者 [batch, 80, 70, 70]
        x = self.Conv2d_3b_1x1(x)

        # [batch, 192, 28, 28] 或者 [batch, 192, 68, 68]
        ...
        # [batch, 256, 28, 28] 或者 [batch, 256, 68, 68]
        x = self.Mixed_5b(x)

        # [batch, 288, 28, 28] 或者 [batch, 288, 68, 68]
        x = self.Mixed_5c(x)

        # [batch, 288, 28, 28] 或者 [batch, 288, 68, 68]
        x = self.Mixed_5d(x)

        # [batch, 768, 13, 13] 或者 [batch, 768, 33, 33]
        x = self.Mixed_6a(x)
        ...
        # google中的template和search对应的是 [33 和 13]
        # resnet中的template和search对应的是 [35 和 15]
        # 有len(bbox)的是对应template，else对应的是search，会把33x33裁剪(crop_pad)成25x25
        if len(bbox):
            assert bbox.shape[-1] == 4 and bbox.shape[0] == x.shape[0]
            stride = cfg.BACKBONE.STRIDE
            offset = cfg.BACKBONE.OFFSET

            mask = torch.zeros(x.shape[0], 1, x.shape[-2], x.shape[-1]).float().cuda()

            roi = ((bbox + 1 - offset + stride / 2) / stride - 1).int()

            for i in range(x.shape[0]):
                mask[i, :, max(0, roi[i][1]): (min(roi[i][3], 12)),
                    max(0, roi[i][0]): (min(roi[i][2], 12))] = 1
            x *= mask
...
if __name__ == "__main__":
    ...
    search_var = torch.FloatTensor(1, 3, 287, 287).cuda()
    model = model.cuda()
    bbox = torch.FloatTensor([[37.5, 45, 84.5, 77]]).cuda()
    t = model(template_var, bbox)
    print("-----------------------------")
    s = model(search_var)
