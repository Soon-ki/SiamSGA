...
class SiamGATTracker(SiameseTracker):
    ...
    def track(self, img):
        ...
        if True:
            cx_ = self.center_pos[0]
            cy_ = self.center_pos[1]
            x_ = np.linspace(1, cfg.TRAIN.SEARCH_SIZE, cfg.TRAIN.SEARCH_SIZE) - 1 - cx_
            y_ = np.linspace(1, cfg.TRAIN.SEARCH_SIZE, cfg.TRAIN.SEARCH_SIZE) - 1 - cy_
            X, Y = np.meshgrid(x_, y_)
            dist = np.sqrt(np.square(X) + np.square(Y)).astype(np.float32)
            dist_map = torch.from_numpy(np.expand_dims(dist, axis=(0, 1)))
